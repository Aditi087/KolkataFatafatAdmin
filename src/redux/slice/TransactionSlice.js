...
export const viewWalletTransaction = createAsyncThunk(
...
export const TransactionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(viewWalletTransaction.pending, (state) => {
        state.status = 'loading';
        state.isLoading = true;
      })
      .addCase(viewWalletTransaction.fulfilled, (state, { payload }) => {
        state.status = 'success';
        state.isSuccess = true;
        // console.log('payload', payload);
        state.wallet_transaction_list = payload?.response?.message;
      })
      .addCase(viewWalletTransaction.rejected, (state, action) => {
        state.status = 'failed';
        state.isError = true;
        state.isSuccess = false;
      })
      ...
      .addCase(
        updateWalletTransactionStatus.fulfilled,
        (state, { payload }) => {
          state.status = 'success';
          state.isSuccess = true;
          // console.log('payload', payload);
